...
module.exports = class orgController {
  ...
  static async getAllOrgs(req, res) {
    ...
    try {
      connect.query(query, function (err, results) {
        ...
        return res
          .status(200)
          .json({ message: "Lista de organizadores:", results });
      });
    } catch (error) {
  ...
  static async updateOrg(req, res) {
    ...
    const query =
      "UPDATE organizador SET nome=?, email=?, senha=?, telefone=? WHERE id_organizador = ?";
    ...
    try {
      ...
      console.error("Erro ao executar consulta", error);
      return res.status(500).json({ error: "Erro interno ao servidor" });
    }
  }
